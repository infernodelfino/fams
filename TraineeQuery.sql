...
SELECT * FROM dbo.trainee WHERE account LIKE 'LinhNT'

--AVG
SELECT COUNT(status) AS [number of status], t.status
	FROM dbo.trainee AS t
	GROUP BY t.status

--group by, having
SELECT id
	FROM dbo.trainee AS t
	WHERE t.status LIKE 'Waiting for Allocation'
	GROUP BY id
	HAVING t.id < 10

--create INDEX
CREATE INDEX idx_traineeId ON dbo.trainee(id)
